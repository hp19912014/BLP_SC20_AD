...
#define NUM_SM_COMPUTE_tmp 78
#define NUM_SM_COMPUTE_y   78

#define IN_CHUNK_SIZE 32
#define IN_CHUNK NX/IN_CHUNK_SIZE 			
#define IN_CHUNK_OFFSET OFFSET*IN_CHUNK_SIZE*4  		

#define BUFFER_NX 8192
#define BUFFER_NX_MASK BUFFER_NX-1
#define BUFFER_NY 65536LL
#define BUFFER_INDEX_NUM BUFFER_NX*BUFFER_NY
#define BUFFER_INDEX_MASK BUFFER_INDEX_NUM-1
#define BUFFER_VECTOR_NUM BUFFER_INDEX_NUM/4
#define BUFFER_VECTOR_MASK BUFFER_VECTOR_NUM-1
#define BUFFER_FLAG_NUM BUFFER_NX
#define FLAG_NUM NY*2
#define FLAG_NUM_PER_SM FLAG_NUM/4
...
__global__ void atax_kernel(DATA_TYPE *A, DATA_TYPE *A_host,  DATA_TYPE *x, DATA_TYPE *y, DATA_TYPE *tmp, int *flag_A, int* flag_tmp)
{
	...
	if (blockIdx.x < NUM_SM_HtoD ){  //copy kernel HtoD
		...
		int chunk_offset;
		for (int64_t i = 0; i<IN_CHUNK;i++ ){

			if ( idx < IN_CHUNK_SIZE ){
                                int flag_id = IN_CHUNK_SIZE*i+idx;
                                if (flag_id > BUFFER_FLAG_NUM){
                                        //while( (atomicOr(&inflag[flag_id - BUFFER_FLAG_NUM],0) != 0 ))
                                        while (flag_global_read(flag_A, (flag_id - BUFFER_FLAG_NUM) )>0)
                                                {
                                                //dummy_comp();
                                                }
                                }
			...
			for (int64_t k = (chunk_offset+bidx*1024+idx);k < ( chunk_offset+IN_CHUNK_OFFSET ) ; k+= OFFSET ){
			...
			if ( idx < IN_CHUNK_SIZE ){
            		atomicAdd(&flag_A[IN_CHUNK_SIZE*i+idx], FLAG_NUM_PER_SM);
     		   	}
		}
	} else if (blockIdx.x < (NUM_SM_HtoD + NUM_SM_COMPUTE_tmp)){
	// compute tmp

		int64_t i = (blockIdx.x - NUM_SM_HtoD) * blockDim.x + threadIdx.x;
        	if (i < NX)
        	{
			while(  flag_global_read(flag_A, i) != FLAG_NUM );

                	for(int64_t j=0; j < NY; j++)
               		{
                        	tmp[i] += A[(i & BUFFER_NX_MASK) * NY + j] * x[j];
               		}
			

			__threadfence();
			atomicOr(&flag_tmp[i],1);
			atomicAdd(&flag_A[i],-NY);
        	}
	...
	int64_t j = (blockIdx.x - NUM_SM_HtoD - NUM_SM_COMPUTE_tmp) * blockDim.x + threadIdx.x;
			...
			while(  flag_global_read(flag_tmp,i) == 0 );
                       	...
                       	y[j] += A[(i & BUFFER_NX_MASK) * NY + j] * tmp[i];

			atomicAdd(&flag_A[i],-1);
                	}	
        	}
	}
}
