...
#define NUM_SM_COMPUTE_tmp 32
#define NUM_SM_COMPUTE_y   32
...
#define NUM_COMPUTE_BLOCK NX/DIM_THREAD_BLOCK_X     
#define COPY_CHUNK_NUM IN_CHUNK/NUM_COMPUTE_BLOCK  	
#ifndef M_PI
#define M_PI 3.14159
#endif
...
__global__ void atax_kernel(DATA_TYPE *A, DATA_TYPE *A_host,  DATA_TYPE *x, DATA_TYPE *y, DATA_TYPE *tmp, int *flag_A, int* flag_tmp, int* devtaskid)
{
	...
	if (blockIdx.x < NUM_SM_COMPUTE_tmp ){  //copy kernel HtoD
		...
		*/
		int chunk_offset = taskid*1024*4096*2;

                for (int k = (chunk_offset+idx);k < ( chunk_offset+1024*4096*2 ) ; k+= OFFSET ){
                                reinterpret_cast<double2*>(A)[k] = reinterpret_cast<double2*>(A_host)[k];
                        }
